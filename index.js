...
app.get("/random", (req, res) => {
  ...
  //I initailly used: res.send(jokes[randomNumber]);

});

//2. GET a specific joke
app.get(`/jokes/:id`, (req, res) => {
  //The line below is used to access URL Path Parameters.(When you want something specific) using express
...
app.get("/filter", (req, res) => {
  ...
  const foundJokes = jokes.filter((joke) => joke.jokeType === jokeType );
  res.json(foundJokes);
});
...
app.post("/jokes", (req, res) =>{
  ...
  const newJoke = {id: jokes.length + 1, jokeText: text, jokeType: type};
  jokes.push(newJoke);
  res.json(newJoke);
...
app.delete("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const searchIndex = jokes.findIndex((joke) => joke.id === id);
  const deletedJoke = delete jokes[searchIndex];
